...
<head>
    ...
    <!--anime citat preko lika-->
    <script>
       function getQuoteByCharacter(characterName) {
      fetch(`baza.json`)
        .then(response => response.json())
        .then(quotes => {
          const quotesByCharacter = quotes.filter(quote => quote.character.toLowerCase() === characterName.toLowerCase());
          if (quotesByCharacter.length > 0) {
            const randomQuote = quotesByCharacter[Math.floor(Math.random() * quotesByCharacter.length)];
            ...
            document.getElementById("quote").textContent = "Nema pronađenih citata vezano uz tog lika. Ponovno napiši ime ili odaberi neko drugo.";
        ...
        document.addEventListener("DOMContentLoaded", function() {
          document.getElementById("new-quote-btn").addEventListener("click", function() {
            ...
            getQuoteByCharacter(characterName);
          });
        });
    </script>
</head>
<body>
  ...
  <div class="random">
    ...
    Upiši ime lika: 
    <input type="text" id="character-input" placeholder="npr. Eren Yeager">
